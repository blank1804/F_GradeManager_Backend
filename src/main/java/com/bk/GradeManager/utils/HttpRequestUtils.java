...
import com.bk.GradeManager.model.core.HttpResponse;
...
public class HttpRequestUtils {
	...
	public static HttpResponse httpGet(String authorization, String url, Map<String, String> param, OkHttpClient client)
		...
		Request request = new Request.Builder().url(buildUrl).get()
	...
	public static HttpResponse httpGet(String authorization, String url, Map<String, String> param, OkHttpClient client, String lang)
			throws Exception {
		HttpUrl.Builder urlBuilder = HttpUrl.parse(url).newBuilder();
	...
	public static HttpResponse httpPost(String authorization, String url, String jsonParam, OkHttpClient client)
			throws Exception {
	...
	public static HttpResponse httpPost(String authorization, String url, String jsonParam, OkHttpClient client, String lang)
			throws Exception {
				log.debug("<< {} << {} ", url, jsonParam);
		HttpUrl.Builder urlBuilder = HttpUrl.parse(url).newBuilder();
		String buildUrl = urlBuilder.build().toString();
		...
		Request request = new Request.Builder().url(buildUrl).post(body)
				.header("Authorization", authorization)
				.header("Accept", "application/json")
				.header("Content-type", "application/json; charset=utf-8")
				.header("language", lang)
//				.header("company", UserUtils.getCompanyCode())
				.build();
		try (Response response = client.newCall(request).execute()) {
			int responseCode = response.code();
			String responseBody = response.body().string();
